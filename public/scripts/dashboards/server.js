...
function plot_client_rssi_time_series(canvasId, data) {
  plot_time_series(canvasId, data, "# RSSI per packet (dBm)")
}
...
function plot_dashboard(data) {
  ...
  for (let section of dashboardSections) {
    ...
    if (section.id == "general") {
      ...
      const canvasIds = [
        section.id + "_client_rssi_time_series",
      ]
      ...
      plot_client_rssi_time_series(canvasIds[0], data.raw.rssi[section.id]);
    }
  }
}
